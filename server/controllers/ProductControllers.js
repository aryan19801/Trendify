...
export const getAllProducts = async (req,res)=>{
   try {
    ...
    console.log(`Error in the productContollers`,error.message);
    res.status(500).json({message:"error in productcontroller",error:error.message});
   }
};
    ...
    try { const product  = await Product.findById(req.params.id);
      ...
      if(product.image){
        ...
        try {
            ...
            console.log("imaghe deleted from the cloudinary");
        } catch (error) {
...
async function updatedProductCache(){
    ...
    try {
        ...
        console.log("error in the update cache function ")
    }
}
